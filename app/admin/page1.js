...
import { saveToSheet, G_SCRIPT_URL } from "@/utils/googleSheets";
...
export default function AdminPage() {
  ...
  // Control de pestañas
  const [activeTab, setActiveTab] = useState("invitados"); // "invitados" o "musica"

  const [phoneData, setPhoneData] = useState({ codArea: "", nroLocal: "" });
  ...
  const getColorEstado = (estado) => {
    switch (estado?.toLowerCase()) {
      case "confirmado": return "text-green-400 font-bold";
      case "cancelado": return "text-red-400 font-bold";
  ...
  const cargarInvitados = async () => {
    ...
    try {
      // Modificación Paso A: Enviamos la password en la URL
      const res = await fetch(`${G_SCRIPT_URL}?pass=${password}`);
      const data = await res.json();
      
      if (data.error === "Unauthorized") {
        alert("Sesión no autorizada o clave incorrecta en el servidor.");
  ...
  const cargarMusica = async () => {
    setLoadingLista(true);
    try {
      // Modificación Paso A: Enviamos la password y la acción en la URL
  ...
  const eliminarInvitado = async (idInvitado) => {
    if (!confirm("¿Estás seguro de que quieres eliminar a este invitado?")) return;
    setStatus("Eliminando...");
    const exito = await saveToSheet({ action: "delete", type: "individual", id: idInvitado }, "delete");
    if (exito) {
      setTimeout(() => {
        cargarInvitados();
        setStatus("✅ Eliminado.");
  ...
  const eliminarGrupo = async (idGrupo) => {
    if (!confirm("¿Estás seguro de que quieres eliminar a TODO el grupo familiar?")) return;
    setStatus("Eliminando grupo...");
    const exito = await saveToSheet({ action: "delete", type: "group", id: idGrupo }, "delete");
    if (exito) {
      setTimeout(() => {
        cargarInvitados();
  ...
  const agregarInvitado = () => setInvitados([...invitados, ""]);

  const handleNombreChange = (index, value) => {
    const nuevosInvitados = [...invitados];
    nuevosInvitados[index] = value;
  ...
  const handleSubmit = async (e) => {
    ...
    const nombreFamilia = invitados[0]; 
    const idGrupoAuto = Math.floor(Date.now() / 1000).toString().slice(-5);
    const telefonoCompleto = `549${phoneData.codArea.trim()}${phoneData.nroLocal.trim()}`;

    const slugAuto = nombreFamilia
      .toLowerCase()
      .trim()
    ...
    const baseInvitacionUrl = window.location.origin + "/i/" + slugAuto;
    const mensajeWA = `¡Hola ${nombreFamilia}! Queremos invitarlos a mis 15 años. Aquí tienen su invitación personalizada: ${baseInvitacionUrl}`;
    const linkWhatsAppFinal = `https://wa.me/${telefonoCompleto}?text=${encodeURIComponent(mensajeWA)}`;

    const filas = invitados.map((nombre, index) => [
      `${idGrupoAuto}-${index + 1}`, 
      idGrupoAuto, 
      nombre, 
      nombreFamilia, 
    ...
    const exito = await saveToSheet(filas);

    if (exito) {
      setStatus(`✅ Guardado.`);
      setLastWALink(linkWhatsAppFinal); 
      setPhoneData({ codArea: "", nroLocal: "" });
      setInvitados([""]);
      cargarInvitados();
    } else {
      setStatus("❌ Error");
        ...
        {activeTab === "invitados" ? (
          <>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 items-start">
              <div className="lg:col-span-2 bg-white/5 p-4 md:p-6 rounded-xl border border-white/10 shadow-2xl">
                <div className="mb-4 text-center lg:text-left">
                  <h2 className="text-sm font-serif italic text-[#c0c0c0]">Nuevo Invitado</h2>
                </div>

                <form onSubmit={handleSubmit} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-[9px] uppercase text-white/30 tracking-tighter">Cod. Área</label>
                      <input type="text" placeholder="11" className="w-full p-2 bg-black border border-white/10 rounded text-sm focus:border-[#c0c0c0] outline-none" value={phoneData.codArea} onChange={(e) => setPhoneData({...phoneData, codArea: e.target.value})} required />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] uppercase text-white/30 tracking-tighter">Número</label>
                      <input type="text" placeholder="12345678" className="w-full p-2 bg-black border border-white/10 rounded text-sm focus:border-[#c0c0c0] outline-none" value={phoneData.nroLocal} onChange={(e) => setPhoneData({...phoneData, nroLocal: e.target.value})} required />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-[9px] uppercase text-white/30 tracking-tighter">Integrantes</label>
                    <div className="max-h-32 overflow-y-auto space-y-2 pr-1">
                      {invitados.map((nombre, index) => (
                        <input key={index} placeholder={index === 0 ? "Nombre Invitado" : `Invitado ${index + 1}`} className="w-full p-2 bg-white/5 border border-white/10 rounded text-sm outline-none focus:border-[#c0c0c0]" value={nombre} onChange={(e) => handleNombreChange(index, e.target.value)} required />
                      ))}
                    </div>
                    <button type="button" onClick={agregarInvitado} className="text-[9px] text-[#c0c0c0] bg-white/5 px-2 py-1 rounded border border-white/10">
                      + INTEGRANTE
                    </button>
                  </div>

                  <button className="w-full bg-[#c0c0c0] text-black py-2.5 rounded font-bold uppercase text-[10px] tracking-widest hover:bg-white shadow-lg">
                    Guardar Nuevo Grupo
                  </button>

                  {status && (
                    <div className="p-2 bg-white/5 border border-[#c0c0c0]/20 text-center rounded text-[10px] text-[#c0c0c0]">
                      {status}
                      {lastWALink && <a href={lastWALink} target="_blank" className="block mt-1 text-green-400 underline">WhatsApp Link ↗</a>}
                    </div>
                  )}
                </form>
              </div>
            ...
            <div className="bg-white/5 p-4 md:p-6 rounded-xl border border-white/10">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-serif italic text-[#c0c0c0]">Lista</h2>
                <button onClick={cargarInvitados} disabled={loadingLista} className="text-[9px] bg-white/10 px-3 py-1.5 rounded uppercase font-bold tracking-widest">
                  {loadingLista ? "Cargando..." : "Actualizar"}
                </button>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="text-[9px] uppercase tracking-tighter text-white/30 border-b border-white/10">
                      <th className="py-2 px-2">Invitados</th>
                      <th className="py-2 px-2 text-right">Estado</th>
                      <th className="py-2 px-2 pl-4">Grupo</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {listaGrupos.map((grupo, idx) => (
                      <tr key={idx} className={`${idx % 2 === 0 ? 'bg-white/[0.02]' : 'bg-transparent'}`}>
                          ...
                          <table className="w-full border-collapse">
                            <tbody className="divide-y divide-white/[0.03]">
                              {grupo.integrantes.map((p, pIdx) => (
                                <tr key={pIdx}>
                                  <td className={`py-2 px-2 text-xs ${getColorEstado(p.estado)}`}>
                                    <div className="flex items-center gap-2">
                                      <button onClick={() => eliminarInvitado(p.idInvitado)} className="text-red-500/40 hover:text-red-500 text-[10px]">✕</button>
                                      {p.nombreInvitado}
                                    </div>
                                  </td>
                        ...
                        <td className="py-3 px-2 pl-4 align-top w-1/4 border-l border-white/[0.03]">
                          <p className="font-bold text-white text-xs truncate">{grupo.nombreGrupo}</p>
                          <p className="text-[10px] text-white/30 font-mono">+{grupo.telefono}</p>
                          <div className="mt-2 flex flex-col gap-2">
                            <button onClick={() => eliminarGrupo(grupo.idGrupo)} className="bg-red-500/10 text-red-500 border border-red-500/20 text-[8px] py-1 px-2 rounded w-fit uppercase hover:bg-red-500/20 transition-colors">Eliminar Grupo</button>
                            <a href={`https://wa.me/${grupo.telefono}?text=Invitacion%20XV:%20${typeof window !== 'undefined' ? window.location.origin : ''}/i/${grupo.slug}`} target="_blank" className="text-green-400/60 text-[9px] underline hover:text-green-400">WhatsApp Link ↗</a>
                          </div>
                        </td>
                      </tr>
                    ))}
